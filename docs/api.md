...
- 会话：HttpOnly Cookie `handcraft_session`，采用滑动空闲超时（默认 7 天）与绝对截止（默认 30 天，由 `SESSION_IDLE_DAYS`、`SESSION_ABSOLUTE_DAYS` 配置）。
...
- 已认证请求会自动滑动续期：当空闲截止时间消耗过半时，服务端原子地延长空闲截止并下发新 Cookie，前端无需感知。
- 绝对截止在建会话时固定，任何活动都无法延长；到期后必须重新登录。
- 退出登录立即撤销当前令牌；修改密码会立即撤销该操作员的全部既有令牌（含其他设备），随后需使用新密码重新登录。
- 已退出或已被撤销的会话，即使存在并发刷新请求也不会被复活。
